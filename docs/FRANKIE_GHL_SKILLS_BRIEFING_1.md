...
# SECTION 5: PAUL + CARL — YOUR DEVELOPMENT FRAMEWORK

## What They Are

**PAUL** (Plan-Apply-Unify Loop) — Structured AI-assisted development framework. Every unit of work follows: PLAN → APPLY → UNIFY. No orphan plans. No skipping closure. Acceptance criteria are defined BEFORE work starts. State persists across sessions.

**CARL** (Context Augmentation & Reinforcement Layer) — Dynamic rule injection. Instead of bloating context with static prompts, CARL loads rules just-in-time based on what you're doing. Working on GHL? GHL rules load. Writing copy? Copywriting rules load. Context stays lean.
...
## How To Use Them For This Mission

Every GHL skill you build MUST follow the PAUL loop:

1. **PLAN** — Define the skill's objective, acceptance criteria (Given/When/Then), tasks, and boundaries BEFORE writing anything
2. **APPLY** — Build the skill, test each function against the acceptance criteria
3. **UNIFY** — Create SUMMARY.md documenting what was built vs what was planned, log decisions, update STATE.md

Use CARL to dynamically load relevant rules:
- When building GHL skills → load GHL domain rules (API endpoints, rate limits, auth patterns)
- When writing outreach copy → load Copywriting SKILL.md rules
- When managing pipeline → load Pipeline rules
- Rules unload when you switch domains → context stays clean

## Why This Matters

Without PAUL/CARL, you'll hit the same problems Jay and I hit in chat:
- Context rot (forgetting what was decided 3 messages ago)
- Orphan plans (saying "I'll build X" then never closing the loop)
- Quality degradation (spawning subagents that return 70% quality work)
- State drift (losing track of what's done vs what's pending)

With PAUL/CARL, every skill build has structure, closure, and persistent state.
...
# SECTION 7: YOUR FIRST MOVE

Before building anything, Frankie, do this:

1. **Clone PAUL and CARL:**
```bash
cd ~/frankie-bot
git clone https://github.com/ChristopherKahler/paul.git
git clone https://github.com/ChristopherKahler/carl.git
```
Read both READMEs. Understand the loop. You will use PAUL for every skill build from here forward.

2. **Inventory your existing skills:** `ls ~/frankie-bot/workspace/agent-skills/` — read through every SKILL.md you already have. Know what you've got.

3. **Read your .env:** Confirm GHL_API_KEY and GHL_LOCATION_ID are present and valid.

4. **Test GHL connection:** Make one simple API call to verify credentials work:
```bash
...
5. **Make your decision:** Option A, B, or C. Tell Jay WHY.

6. **Run /paul:init** — Initialize PAUL in the frankie-bot project. Every skill build follows the loop from here.

7. **Build Tier 1 first.** The money skills. Everything else is support infrastructure.

8. **Test each skill before moving to the next.** No moving forward until the current skill passes its own acceptance criteria (PAUL-style Given/When/Then).

---

# SECTION 8: WHAT JAY EXPECTS
